...
@php
    ...
    $postCategories = $post->categories->pluck('name')->all();
@endphp
...
@section('content')
    <main class="mx-auto w-full max-w-6xl px-6 pb-20 pt-10" aria-labelledby="post-title">
        ...
        <div class="grid grid-cols-1 gap-8 lg:grid-cols-[minmax(0,1.25fr)_minmax(320px,0.75fr)]">
            <article
                x-data="slideInOnScroll()"
                x-init="observe($el)"
                :class="isVisible ? 'is-visible' : ''"
                class="overflow-hidden rounded-[2rem] border border-slate-200/80 bg-white/88 shadow-premium backdrop-blur-sm dark:border-slate-700/60 dark:bg-slate-900/88"
            >
                <picture>
                ...
                <div class="space-y-8 p-6 md:p-10" id="article-content">
                    <div class="flex flex-wrap items-center justify-between gap-4">
                        ...
                        <div class="flex flex-wrap gap-2">
                            @foreach ($postCategories as $categoryName)
                                <span class="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-600 dark:border-slate-700/60 dark:bg-slate-950/50 dark:text-slate-300">
                                    {{ $categoryName }}
                                </span>
                            @endforeach
                        </div>
                    ...
                    <header class="space-y-4">
                        <div class="flex flex-wrap items-center gap-3 text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400" style="font-family: var(--font-body);">
                            <time datetime="{{ $post->published_at?->toDateString() }}">{{ $post->published_at?->format('M d, Y') }}</time>
                            <span aria-hidden="true">•</span>
                            <span>{{ $post->reading_time_minutes }} min read</span>
                            @if ($postCategories)
                                <span aria-hidden="true">•</span>
                                <span>{{ count($postCategories) }} topic{{ count($postCategories) === 1 ? '' : 's' }}</span>
                            @endif
                        </div>

                        <h1 id="post-title" class="text-3xl font-semibold tracking-tight text-slate-900 md:text-5xl" style="font-family: var(--font-display);">
                            {{ $post->title }}
                        </h1>

                        <p class="max-w-3xl text-base leading-relaxed text-slate-600 dark:text-slate-300 md:text-lg" style="font-family: var(--font-body);">
                            {{ $post->excerpt }}
                        </p>
                    </header>
                    ...
                    <section class="space-y-5 border-t border-slate-200 pt-8 dark:border-slate-700/60">
                            ...
                            <div>
                                @if ($previousPost)
                                    <a
                                        ...
                                        class="portal-link-button flex h-full flex-col items-start justify-center text-left"
                                    >
                            ...
                            <div class="sm:text-right">
                                @if ($nextPost)
                                    <a
                                        ...
                                        class="portal-link-button flex h-full flex-col items-start justify-center text-left sm:items-end sm:text-right"
                                    >
            ...
            <aside class="space-y-6">
                <section class="rounded-[2rem] border border-slate-200/80 bg-white/88 p-6 shadow-premium backdrop-blur-sm dark:border-slate-700/60 dark:bg-slate-900/88">
                    <p class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">Article Snapshot</p>
                    <div class="mt-4 space-y-4 text-sm text-slate-700 dark:text-slate-300">
                        ...
                        <div class="flex items-center justify-between gap-4">
                            <span class="text-slate-500 dark:text-slate-400">Read time</span>
                            <span class="font-medium">{{ $post->reading_time_minutes }} minutes</span>
                        </div>
                        <div class="flex items-center justify-between gap-4">
                            <span class="text-slate-500 dark:text-slate-400">Topics</span>
                            <span class="font-medium">{{ count($postCategories) }}</span>
                        </div>
                    </div>
                </section>
                ...
                @if ($relatedPosts->isNotEmpty())
                    <section class="rounded-[2rem] border border-slate-200/80 bg-white/88 p-6 shadow-premium backdrop-blur-sm dark:border-slate-700/60 dark:bg-slate-900/88">
                        <p class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">Related Reads</p>
                        <div class="mt-4 space-y-3">
                            @foreach ($relatedPosts as $relatedPost)
                                <a href="{{ route('blog.show', $relatedPost->slug) }}" class="block rounded-2xl border border-slate-200/80 bg-slate-50/90 p-4 transition hover:-translate-y-0.5 hover:border-orange-300 hover:bg-orange-50 dark:border-slate-700/60 dark:bg-slate-950/40 dark:hover:bg-slate-900">
                                    <p class="text-sm font-semibold tracking-tight text-slate-900 dark:text-slate-100">{{ $relatedPost->title }}</p>
                                    <p class="mt-1 line-clamp-2 text-xs leading-relaxed text-slate-600 dark:text-slate-300">{{ $relatedPost->excerpt }}</p>
                                </a>
                            @endforeach
                        </div>
                    </section>
                ...
                <section class="rounded-[2rem] border border-orange-200 bg-orange-50 p-6 shadow-premium dark:border-orange-400/30 dark:bg-orange-500/10">
                    <p class="text-xs font-semibold uppercase tracking-[0.18em] text-orange-700 dark:text-orange-200">Want more?</p>
                    <p class="mt-3 text-sm leading-relaxed text-orange-900/90 dark:text-orange-100/90">
                        Browse the full blog archive for more implementation notes, product updates, and engineering write-ups.
                    </p>
                    <a href="{{ route('blog.index') }}" class="mt-4 inline-flex rounded-xl bg-orange-600 px-4 py-2 text-xs font-semibold uppercase tracking-[0.16em] text-white transition hover:bg-orange-700">
                        Open Blog Archive
                    </a>
                </section>
            </aside>
